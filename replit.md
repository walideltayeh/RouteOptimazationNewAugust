...
- **Route Optimization**: Multi-algorithm TSP solving: nearest-neighbor construction → 2-opt edge reversal → Or-opt segment relocation → 3-opt triple-edge reconnection (for routes ≤50 outlets). Achieves near-optimal route quality within each daily cluster.
- **Visit Frequency Handling (VF1/VF2/VF3/VF4)**: Anchor-aware weekly scheduling. (1) VF3+VF4 outlets cluster into daily zones first as geographic anchors. (2) VF1/VF2 outlets attach to the nearest daily zone centroid. (3) Within each day, outlets are spatially sub-clustered into rotation buckets and assigned to week-patterns: VF4→{1,2,3,4}; VF3→4 patterns each skipping one week ({1,2,3} {1,2,4} {1,3,4} {2,3,4}) for balanced weekly load; VF2→{1,3}|{2,4} biweekly alternation; VF1→{1}|{2}|{3}|{4} once per month. Each outlet is always visited on the same day-of-week (industry journey-plan rule). Coverage and same-day consistency are validated end-to-end. Powered by `buildAnchorAwareSchedules` shared helper used by both initial generation and re-optimization paths.
- **Schedule Generation**: Two-level geographic scheduling: (1) global clustering assigns outlets to reps, (2) per-rep **multi-strategy competitive clustering** creates exactly `workingDaysPerWeek` geographically-tight daily groups. **Four initialization strategies** (Sweep angular, Maximin farthest-point, Grid-based, K-Means++) each run through Constrained K-Means (Bradley et al., 2000) → exhaustive pair-swap optimization → 3-way chain-move optimization → **route-length boundary refinement** (swaps boundary outlets between adjacent clusters when the move reduces actual TSP route length). Tournament winner is selected by **true drive-distance cost** (per-cluster nearest-neighbor + 2-opt), not centroid distance — directly minimizing what reps actually drive each day. Respects visit frequency constraints (VF4=weekly, VF2=biweekly alternating weeks 1+3/2+4, VF1=monthly one week only).
- **Analytics & Visualization**: Interactive map for territory visualization, analytics dashboard, and ML insights. Rep Map page supports per-VF filtering (VF1/VF2/VF3/VF4 checkboxes) and a "Color outlets by" toggle (Day vs Visit Frequency) using a fixed palette (VF1 slate, VF2 blue, VF3 amber, VF4 brand red). VF coloring applies to schedule view, universe view, both GeoJSON and DOM marker rendering paths, and outlet popups.
