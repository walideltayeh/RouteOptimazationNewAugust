...
- **Schedule Generation**: Two-level geographic scheduling: (1) global clustering assigns outlets to reps, (2) per-rep **multi-strategy competitive clustering** creates exactly `workingDaysPerWeek` geographically-tight daily groups. Three initialization strategies (Sweep angular, Maximin farthest-point, Grid-based) each run through Constrained K-Means (Bradley et al., 2000), exhaustive pair-swap optimization, and 3-way chain-move optimization. The result with lowest total intra-cluster distance wins. Respects visit frequency constraints (VF4=weekly, VF2=biweekly alternating weeks 1+3/2+4, VF1=monthly one week only).
